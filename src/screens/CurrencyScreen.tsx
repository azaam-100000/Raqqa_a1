...
const CurrencyScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (localRates && Object.keys(localRates).length > 0) {
             if (isGuest && sharedCity && localRates[sharedCity]) {
    ...
    const currentCityRates = localRates && localRates[selectedCity] ? localRates[selectedCity] : null;
...
📲 *تطبيق سوق محافظة الرقة*
 اضغط على الرابط لمتابعة آخر الأسعار لجميع المحافظات، وتصفح أكبر سوق لكل شي جديد ومستعمل في الرقة!
    ...
    const renderLocalMarketSection = () => {
        ...
        if (!localRates || Object.keys(localRates).length === 0) {
             return <div className="text-center text-gray-500 dark:text-zinc-400 py-10">لا توجد أسعار محلية متاحة حالياً.</div>;
        }

        if (!currentCityRates) return null;
