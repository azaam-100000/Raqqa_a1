...
const ChatScreen: React.FC = () => {
    ...
    const handleSendMessage = async (content: string | null, imageUrl: string | null = null, audioUrl: string | null = null) => {
        ...
        // 3. Insert into DB and get the real message back
        ...
        if (error) {
            ...
            // 4a. On error, remove the optimistic message
            setMessages(current => current.filter(m => m.id !== tempId));
        } else if (insertedMessage) {
            // 4b. On success, replace the optimistic message with the real one
    ...
    const handleCancelRecording = () => {
        if (mediaRecorderRef.current && isRecording) {
            // Stop the recorder but prevent onstop from processing the data
