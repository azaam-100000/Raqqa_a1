...
const StoreCard: React.FC<{ store: Store }> = ({ store }) => {
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 flex items-center gap-4 transition-colors hover:border-slate-600">
        <Link to={`/store/${store.id}`} className="flex-shrink-0">
            <div className="w-24 h-24 rounded-lg bg-slate-700 overflow-hidden flex items-center justify-center">
                {imageUrl ? (
        ...
        <div className="flex-1 min-w-0">
            <Link to={`/store/${store.id}`} className="hover:underline">
                <h2 className="font-bold text-white truncate text-lg">{store.name}</h2>
            </Link>
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-slate-400 text-sm mt-2">
